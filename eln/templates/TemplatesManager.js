...
import getChangedGroups from '../../rest-on-couch/getChangedGroups';
import showRecordInfo from '../../rest-on-couch/showRecordInfo';
...
export default class TemplatesManager {
   ...
   * @param {object[]} [options.categories=[{value: 'chemical', description: 'Chemical'}]]
  ...
  async editTemplateAccess(entry) {
    ...
    try {
      ...
      UI.showNotification(/** @type {Error} */ (e).message, 'error');
    }
  }

  async getTemplate(entry) {
